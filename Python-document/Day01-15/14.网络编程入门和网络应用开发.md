...
```
json.loads :把str类型转成dict个格式

json.load :从文件中读取str，返回也是str类型

json.dumps:把dict或者其他格式转成json格式

json.dump：把dict转成json格式，写入文件
```
...
data = {'a': 1,'b': 2} 
# 1
data = json.dumps(datas)headers = {'Content-Type': 'application/json'}response = requests.post(url, data=data, headers=headers)
# 2-json参数会自动将字典类型的对象转换为json格式 requests.post(url, json=data)
```
